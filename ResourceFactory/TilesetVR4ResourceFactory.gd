class_name TilesetVR4ResourceFactory
extends RefCounted
...
func create_resource(input_file: String, output_dir: String):
	...
	var vr4_data: Array = json.data
	
	var tileset_vr4_resource := TilesetVR4Resource.new()
	...
	var save_result = ResourceSaver.save(tileset_vr4_resource, resource_path)
	if save_result != OK:
		print("NOK: ", save_result)

	# copy images as it's direct used by the resource loader
	# TODO like in GraphicResourceFactory
